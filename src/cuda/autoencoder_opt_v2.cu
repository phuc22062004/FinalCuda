...
// ============================================================================
// GPU OPTIMIZED V2 - KERNEL FUSION + ARGMAX POOL
// ============================================================================
// Optimization Focus: Reduce kernel launches and remove atomicAdd
// Technique 1: Fuse Conv+Bias+ReLU into single kernel
// Technique 2: Store argmax indices in MaxPool, no atomicAdd in backward
// Expected: 1.5-2x speedup from reduced overhead + better memory pattern
// ============================================================================

// V2: Fused Conv2D + Bias + ReLU kernel
// Combines 3 operations: convolution, bias add, ReLU activation
// Saves 2 kernel launches + 1 global memory write/read per layer
__global__ void conv2d_relu_kernel_v2(
    const float* __restrict__ input,   // [C_in, H_in, W_in]
    const float* __restrict__ weight,  // [C_out, C_in, K, K]
    const float* __restrict__ bias,    // [C_out]
    float* __restrict__ output,        // [C_out, H_out, W_out] - post-ReLU
...
// V2: MaxPool with argmax indices (no atomicAdd in backward)
__global__ void maxpool_kernel_v2(
    const float* __restrict__ input,   // [C, H, W]
    float* __restrict__ output,        // [C, H/2, W/2]
    int* __restrict__ indices,         // [C, H/2, W/2] - stores argmax position
    int C, int H, int W)
{
    int c = blockIdx.x;
    int oh = blockIdx.y * blockDim.y + threadIdx.y;
    ...
    float val1 = input[c * H * W + ih * W + (iw + 1)];
    if (val1 > max_val) { max_val = val1; max_idx = ih * W + (iw + 1); }
    
    float val2 = input[c * H * W + (ih + 1) * W + iw];
    if (val2 > max_val) { max_val = val2; max_idx = (ih + 1) * W + iw; }
    
    float val3 = input[c * H * W + (ih + 1) * W + (iw + 1)];
    if (val3 > max_val) { max_val = val3; max_idx = (ih + 1) * W + (iw + 1); }
    
    output[c * H_out * W_out + oh * W_out + ow] = max_val;
    indices[c * H_out * W_out + oh * W_out + ow] = max_idx;
}
...
// V2: MaxPool backward using stored indices (no atomicAdd!)
__global__ void maxpool_backward_kernel_v2(
    const float* __restrict__ grad_output,  // [C, H/2, W/2]
    const int* __restrict__ indices,         // [C, H/2, W/2]
    ...
    int C, int H, int W)
{
    ...
    // Get stored argmax index
    int idx_flat = indices[c * H_out * W_out + oh * W_out + ow];
    
    // Pass gradient to that position (no atomic needed!)
    float grad = grad_output[c * H_out * W_out + oh * W_out + ow];
    grad_input[c * H * W + idx_flat] = grad;
}
