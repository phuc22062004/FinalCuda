...
std::vector<float> extract_features_from_encoder(AutoencoderCPU& ae, const float* input_chw) {
...
int main(int argc, char** argv) {
    ...
    std::string output_train = "train_features_cpu.libsvm";
    std::string output_test = "test_features_cpu.libsvm";
    std::string scaler_path = "scaler_z_cpu.bin";
    std::string cache_path = "train_cache_cpu.bin";
    ...
    std::cout << "=== CPU Feature Extraction for SVM (With Z-Score Scaling) ===\n";
    ...
    std::cout << "====================================================\n";
    std::cout << "SUMMARY:\n";
    std::cout << "  Total CPU Extraction: " << ((extract_us + test_extract_us)/1000000) << "s\n";
